@echo off
REM run-sdlc_20_planning_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_20_planning_v1 workflow (plugin package):
REM   Generates solution architecture plan from approved requirements document.
REM   Loads the workflow definition from workflows/sdlc_20_planning_v1/workflow.toml.
REM
REM Runtime jobs and sidecars are stored under %USERPROFILE%\.ukbe-runner\jobs\
REM
REM Typical workflow:
REM   1. Edit the variables below (especially REQ_FILE)
...
set "TEMPLATE_GROUP=sdlc_20_planning_v1"
...
REM Approved requirements document (filename only, e.g., REQ-20260722-001_console-sdlc10-support.md)
REM Must exist in docs/repo/agent_runner/sdlc/delivery/10_requirements/
set "REQ_FILE=REQ-20260723-001_console-sdlc10-support.md"
...
set "SEED_FLAGS="
if not "!REQ_FILE!"=="" (
    set "REQ_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\10_requirements\!REQ_FILE!"
    if not exist "!REQ_PATH!" (
        echo ERROR: Requirements file not found: !REQ_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set REQ_FILE=!REQ_PATH!"
)
...
if not "!REQ_FILE!"=="" echo  Req File:        !REQ_FILE!
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-20-planning-status-!RANDOM!.txt"
