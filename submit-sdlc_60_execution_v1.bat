@echo off
REM submit-sdlc_60_execution_v1.bat - Edit the variables below, then run.
REM
REM Submits a new backend job for the sdlc_60_execution_v1 workflow:
REM   ukbe-run-agent submit --workflow-name sdlc_60_execution_v1 ...
REM
REM This workflow executes tasks following approved implementation docs,
REM producing an approved EXEC_FILE document.
...
set "WORKFLOW_NAME=sdlc_60_execution_v1"
set "INITIATIVE_ID="
...
set "INPUT_FLAGS="
if not "!IMPL_FILE!"=="" (
    set "IMPL_PATH=%~dp0docs\repo\agent_runner\sdlc\delivery\50_implementations\!IMPL_FILE!"
    if not exist "!IMPL_PATH!" (
        echo ERROR: Implementation file not found: !IMPL_PATH!
        pause
        exit /b 1
    )
    set "INPUT_FLAGS=--input IMPL_FILE=!IMPL_PATH!"
)
