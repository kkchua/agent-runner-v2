...
def build_output_paths(*, job_id: str = "{job_id}", mode: str = "{mode}", loop_iteration: int = 0) -> dict[str, str]:
    """Build output path mappings for 02_agent_runner_platform_v1 workflow.
    
    This function returns a dictionary mapping artifact keys to their
    relative paths within the repository, following the platform-specific
    governance folder structure.
    
    Args:
        job_id: Job identifier for path construction.
        mode: Execution mode (e.g., "manual", "daemon").
        loop_iteration: Current loop iteration for refine steps.
...
def build_context_extensions(
    *,
    ...
    """Build context extensions for 02_agent_runner_platform_v1 workflow.
    
    This function provides additional context variables needed by the
    platform core workflow prompts, including paths to Layer 1 governance,
    masterplan documents, and output artifact paths.
