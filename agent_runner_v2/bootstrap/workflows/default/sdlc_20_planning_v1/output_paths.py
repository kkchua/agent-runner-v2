"""Output paths for sdlc_20_planning_v1 workflow.

This module defines the artifact path mappings for the planning workflow,
...
def build_output_paths(*, job_id: str = "{job_id}", mode: str = "{mode}") -> dict[str, str]:
    """Build output path mappings for sdlc_20_planning_v1 workflow.
    ...
    return {
        # Plan document (draft stage)
        "PLAN_FILE": f"{delivery_base}/plans/PLAN-{date_str}-001_{{slug}}.md",

        # Review document (evidence artifact)
        "REVIEW_FILE_SUGGESTED": f"{delivery_base}/plans/{job_id}-plan-review.md",
    }
