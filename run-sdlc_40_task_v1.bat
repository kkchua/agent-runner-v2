@echo off
REM run-sdlc_40_task_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_40_task_v1 workflow (plugin package):
REM   Generates task specification from approved backlog document.
REM   Loads the workflow definition from workflows/sdlc_40_task_v1/workflow.toml.
REM
REM Runtime jobs and sidecars are stored under %USERPROFILE%\.ukbe-runner\jobs\
REM
REM Typical workflow:
REM   1. Edit the variables below (especially BACKLOG_FILE)
...
set "TEMPLATE_GROUP=sdlc_40_task_v1"
...
set "SEED_FLAGS="
if not "!BACKLOG_FILE!"=="" (
    set "BACKLOG_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\30_backlogs\!BACKLOG_FILE!"
    if not exist "!BACKLOG_PATH!" (
        echo ERROR: Backlog file not found: !BACKLOG_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set BACKLOG_FILE=!BACKLOG_PATH!"
)
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-40-task-status-!RANDOM!.txt"
