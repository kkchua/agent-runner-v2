"""Custom actions for Workflow Builder v3.

Provides:
- validate_package_deterministic: Static analysis of generated package
  files (11 deterministic checks) to catch runtime defects before
  LLM gatekeeper review.
- promote_workflow_package: Deploy 3-part generated package to
  workflows/ directory with enforcement of Standards/ and Specs/.
...
# ---------------------------------------------------------------------------
# Deterministic package validation (Step 16)
# ---------------------------------------------------------------------------
...
def validate_package_deterministic(
    *,
    ...
    """Run 11 static analysis checks on the generated package files.

    Checks performed:
    1. TOML parse validity of workflow.toml
    2. Python syntax validity of context_extensions.py and actions.py
    3. TYPE_CHECKING runtime import detection
    4. Artifact binding consistency (no dangling references)
    5. Action step implementation completeness
    6. Prompt file existence (one .txt per prompt step)
    ...
    10. Specs/ directory exists with at least one .md file
    11. Bidirectional prompt placeholder vs artifact declaration
        consistency
    ...
    standards_str = artifacts.get("STANDARDS_COMPOSITION_STANDARD_FILE", "")
    specs_str = artifacts.get("SPECS_BUILDER_SPEC_FILE", "")
    ...
    standards_path = Path(standards_str) if standards_str else None
    specs_path = Path(specs_str) if specs_str else None
    ...
    # Check 3: TYPE_CHECKING import detection
    if extensions_path and extensions_path.is_file():
        _check_type_checking_imports(extensions_path, findings)
    ...
    # Check 10: Specs/ directory exists with at least one .md file
    if bundle_root:
        _check_specs_directory(bundle_root, findings)

    # Check 11: Bidirectional prompt placeholder vs artifact consistency
    if manifest_data and bundle_root:
        _check_bidirectional_consistency(
            manifest_data, bundle_root, findings
        )
...
def _write_report(
    ...
    report_path = (
        run_root
        / f"VALIDATION_REPORT-{datetime.now().strftime('%Y%m%d')}-001.md"
    )
...
def _check_specs_directory(
    bundle_root: Path, findings: list[dict[str, str]]
) -> None:
    """Check that Specs/ directory exists with at least one .md file.

    The v3 spec requires every generated meta builder to include a
    Specs/ directory with at least one .md file (the embedded builder
    spec). This validates that the embed_builder_spec step completed.
    """
    ...
    if not specs_dir.is_dir():
        findings.append(
            _finding(
                "error",
                "MISSING_SPECS_DIRECTORY",
                (
                    "Specs/ directory not found in output package. "
                    "This is a required v3 innovation: every generated "
                    "meta builder must have a Specs/ directory with "
                    "at least one .md file for self-bootstrapping."
                ),
            )
        )
        return
    ...
    if not md_files:
        findings.append(
            _finding(
                "error",
                "EMPTY_SPECS_DIRECTORY",
                (
                    "Specs/ directory exists but contains no .md files. "
                    "At least one .md file is required for "
                    "self-bootstrapping."
                ),
            )
        )
...
def _check_bidirectional_consistency(
    ...
    """Check bidirectional prompt placeholder vs artifact consistency.

    Check 7 verifies prompt->workflow direction (every placeholder in
    the prompt is declared). This check verifies the reverse: every
    artifact in required_inputs/produces that looks like a placeholder
    IS referenced in the prompt.
    """
    steps = manifest_data.get("step", [])
    if not isinstance(steps, list):
        return
    ...
    for step in steps:
        ...
        # Find declared keys that are not used as placeholders
        # Exclude known runtime-resolved keys
        runtime_keys = {
            "job_id", "seq", "timestamp", "workspace_root",
            "UNRESOLVED", "ARTIFACT_KEY", "SOME_KEY",
        }
        unused_declarations = (declared_keys - placeholders) - runtime_keys

        # Only flag if the declared key looks like a placeholder target
        for key in sorted(unused_declarations):
            if key.endswith("_FILE") and key not in content:
                findings.append(
                    _finding(
                        "warning",
                        "UNUSED_ARTIFACT_DECLARATION",
                        (
                            f"Step '{step.get('name', '')}' declares "
                            f"'{key}' in artifacts but it is not "
                            f"referenced as {{{key}}} in the prompt"
                        ),
                    )
                )
...
def _render_report(
    ...
    # Summary table of all 11 checks
    lines.append("## Check Results")
    lines.append("")
    lines.append("| Check | Description | Status |")
    lines.append("|---|---|---|")
    ...
    check_names = [
        ...
        "9: Standards/COMPOSITION_STANDARD.md existence",
        "10: Specs/ directory exists with at least one .md file",
        "11: Bidirectional prompt placeholder vs artifact consistency",
    ]
    ...
    for i, check_name in enumerate(check_names):
        code = check_codes[i]
        has_finding = any(f["code"] == code for f in findings)
        status = "FAIL" if has_finding else "PASS"
    ...
    if findings:
        ...
        lines.append(
            "No findings. Package passed all 11 deterministic checks."
        )
...
# ---------------------------------------------------------------------------
# Promote workflow package (Step 20)
# ---------------------------------------------------------------------------
...
def promote_workflow_package(
    *,
    ...
    """Deploy the 3-part generated package to workflows/ directory.

    Copies deployable files from the run output directory to the
    target workflows/{slug}/ directory. Enforces that Standards/
    and Specs/ directories are present in the output. If either
    is missing, the action REJECTS with error code
    ...
    for required_dir in ["Standards", "Specs"]:
        ...
        if not src.is_dir():
            return ActionResult(
                status="REJECTED",
                remark=(
                    f"Required output directory '{required_dir}/' not "
                    f"found. v3 requires 3-part output: Standards/, "
                    f"Specs/, workflow files."
                ),
                artifacts={},
