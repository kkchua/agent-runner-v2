...
Workflow Builder v3 is a meta-meta builder that generates composition system workflows (meta builders). Each generated meta builder is itself a composition system with its own composition standard, enabling extensibility and self-bootstrapping.
...
- `template_group`: "workflow_builder_v3"
...
```
Layer 1: Component Schema    -- 8 component types, 16 validation rules
Layer 2: Composition Format  -- 9 binding rules, 6 workflow patterns
Layer 3: Output Format       -- 3-part output, 7 resolution rules, 9 quality checks
```
