...
#### GovernanceFoundationExtensions

**Inherits from**: `WorkflowExtensions`

**Purpose**: Workflow extension hooks for 01_governance_foundation_v1.

**Methods**:

- `register_artifact_keys(*, job_id: str = '{job_id}', mode: str = '{mode}')` -> `dict[str, str]` -- Return artifact key to relative-path mappings.
- `build_context_extensions(*, state: dict[str, Any], step: str, step_cfg: dict[str, Any], ctx: dict[str, str], project_root: Path | None = None)` -> `dict[str, str]` -- Build context extensions for 01_governance_foundation_v1.
- `install_to_global(*, workspace_root, runner_home)` -- Copy L1 governance docs to global runner home.
