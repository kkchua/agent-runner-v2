---
title: "Module Documentation: agent_runner_v2.actions.documentation_validation_core"
template_id: "CB-02"
version: "1.0.0"
doc_type: "system"
authority: "workflow-generated"
scan_policy: "include"
lifecycle_status: "approved"
module_path: "agent_runner_v2/actions/documentation_validation_core.py"
module_area: "actions"
documentation_mode: "full"
owner_doc_path: "docs/repo/codebase/current/02_modules/agent-runner-v2-actions-documentation-validation-core.md"
last_verified_by_change: "sdlc_00_codebase_v1 / SDLC00CB-20260723-786ab7b6 / 2026-07-23T21:25:54+08:00"
created: "2026-07-23T21:25:54+08:00"
owner: "sdlc_00_codebase_v1"
---

# Module Documentation: agent_runner_v2.actions.documentation_validation_core

## 1. Module Overview

### 1.1 Purpose

Auto-generated baseline documentation from repository scan.

### 1.2 Responsibility

This module belongs to the `actions` area and is documented as `full`.

### 1.3 Dependencies

| Dependency | Type | Purpose |
|------------|------|---------|
| `__future__` | stdlib module | imported dependency |
| `dataclasses` | stdlib module | imported dependency |
| `pathlib` | stdlib module | imported dependency |
| `re` | stdlib module | imported dependency |
| `typing` | stdlib module | imported dependency |

## 2. Public API

### 2.1 Classes

#### DocumentationValidationPlan

**Decorators**: `@dataclass`

**Purpose**: Public class


### 2.2 Functions

#### check_file_exists()

**Signature**: `check_file_exists(project_root: Path, rel_path: str)`

**Parameters**:

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `project_root` | `Path` | -- | -- |
| `rel_path` | `str` | -- | -- |

**Returns**: `tuple[bool, str]`

---

#### check_folder_exists()

**Signature**: `check_folder_exists(project_root: Path, rel_path: str)`

**Parameters**:

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `project_root` | `Path` | -- | -- |
| `rel_path` | `str` | -- | -- |

**Returns**: `tuple[bool, str]`

---

#### read_file()

**Signature**: `read_file(project_root: Path, rel_path: str)`

**Parameters**:

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `project_root` | `Path` | -- | -- |
| `rel_path` | `str` | -- | -- |

**Returns**: `str | None`

---

#### has_section()

**Signature**: `has_section(content: str, section: str)`

**Parameters**:

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `content` | `str` | -- | -- |
| `section` | `str` | -- | -- |

**Returns**: `bool`

---

#### has_frontmatter_field()

**Signature**: `has_frontmatter_field(content: str, field: str)`

**Parameters**:

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `content` | `str` | -- | -- |
| `field` | `str` | -- | -- |

**Returns**: `bool`

---

#### validate_documentation_plan()

**Signature**: `validate_documentation_plan(*, project_root: Path, plan: DocumentationValidationPlan)`

**Parameters**:

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `project_root` | `Path` | -- | -- |
| `plan` | `DocumentationValidationPlan` | -- | -- |

**Returns**: `list[ValidationCheck]`

---


### 2.3 Constants / Configuration

No public constants.


## 3. Error Handling

No documented exceptions.


## 4. Testing

### 4.1 Test Coverage

| Test File | Coverage Area |
|-----------|---------------|
| (none) | No test references found |


## 5. Change Log

| Date | Change | Verified By |
|------|--------|-------------|
| 2026-07-23 | Initial baseline generated from repository scan | sdlc_00_codebase_v1 |
