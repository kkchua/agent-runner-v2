"""Actions for sdlc_00_delivery_scaffold_v1 workflow.

Provides the publish step that promotes approved scaffold artifacts
into the active ``sdlc/current/`` location and records a historical
snapshot in ``sdlc/history/<job_id>/``.
"""
...
# ---------------------------------------------------------------------------
# Scaffold artifact registry
# ---------------------------------------------------------------------------
# Each entry: (artifact_key, filename, subdirectory)
...
# ---------------------------------------------------------------------------
# Helper functions (local copies following L2 publish pattern)
# ---------------------------------------------------------------------------
...
def publish_sdlc_scaffold(
    *,
    ...
    del step_cfg
    job_id = str(state.get("job_id") or "SDLC00SCF")
    ...
    # Write manifest
    manifest = {
        "workflow_id": "sdlc_00_delivery_scaffold_v1",
    ...
    artifacts = {"SDLC_SCAFFOLD_PUBLISH_MANIFEST": str(current_manifest.relative_to(project_root)).replace("\\", "/")}
    if meta_rel:
        write_meta_sidecar(
            meta_rel,
...
def _resolve_run_root(context: dict, state: dict, project_root: Path) -> Path | None:
    """Derive the run root from any known scaffold artifact path."""
