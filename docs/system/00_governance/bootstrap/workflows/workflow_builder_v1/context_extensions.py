...
class WorkflowBuilderExtensions(WorkflowExtensions):
    ...
    def register_artifact_keys(
        ...
        return {
            ...
            "TEST_CRITERIA_FILE": f"{run_root}/TEST_CRITERIA-{date_str}-{{seq}}_{{slug}}.md",
            "WORKFLOW_REQUIREMENTS_FILE": f"{run_root}/REQUIREMENTS-{date_str}-{{seq}}_{{slug}}.md",
            "ARTIFACT_CONTRACT_FILE": f"{run_root}/ARTIFACTS-{date_str}-{{seq}}_{{slug}}.md",
            "STEP_ARCHITECTURE_FILE": f"{run_root}/STEPS-{date_str}-{{seq}}_{{slug}}.md",
            "WORKFLOW_MANIFEST_FILE": f"{run_root}/workflow.toml",
            "WORKFLOW_EXTENSIONS_FILE": f"{run_root}/context_extensions.py",
            "WORKFLOW_ACTIONS_FILE": f"{run_root}/actions.py",
            "WORKFLOW_PROMPTS_INDEX_FILE": f"{run_root}/PROMPTS-{date_str}-{{seq}}_{{slug}}.md",
            "WORKFLOW_README_FILE": f"{run_root}/README.md",
            "WORKFLOW_ENV_SAMPLE_FILE": f"{run_root}/.env.sample",
            "WORKFLOW_CONFIG_SAMPLE_FILE": f"{run_root}/config.json.sample",
            "VALIDATION_REPORT_FILE": f"{run_root}/VALIDATION-{date_str}-{{seq}}_{{slug}}.md",
            "REVIEW_FILE_SUGGESTED": f"{run_root}/REVIEW-{date_str}-{{seq}}_{{slug}}.md",
            "GATEKEEP_REQUIREMENTS_FILE": f"{run_root}/GATEKEEP-REQ-{date_str}-{{seq}}_{{slug}}.md",
            "GATEKEEP_ARTIFACTS_FILE": f"{run_root}/GATEKEEP-ART-{date_str}-{{seq}}_{{slug}}.md",
            "GATEKEEP_STEPS_FILE": f"{run_root}/GATEKEEP-STEPS-{date_str}-{{seq}}_{{slug}}.md",
            "GATEKEEP_PACKAGE_FILE": f"{run_root}/GATEKEEP-PKG-{date_str}-{{seq}}_{{slug}}.md",
            "WORKFLOW_PACKAGE_DIR_FILE": f"{run_root}/WORKFLOW_PACKAGE_DIR-{date_str}-{{seq}}_{{slug}}",
        }
