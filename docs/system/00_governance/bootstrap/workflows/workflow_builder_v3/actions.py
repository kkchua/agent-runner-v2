...
- Added check 9 to validate_package_deterministic: verifies
  Standards/COMPOSITION_STANDARD.md exists (v3 innovation).
- Added _check_composition_standard_file helper function.
- Added "Standards" and "Specs" to copy_dirs in
  promote_workflow_package (spec Section 5.4, TC-OW-020).

Refinement Summary (Iter 2):
- Verified: no TYPE_CHECKING blocks, 2 @action decorators correct,
  all returns are ActionResult, Standards check path resolves to
  output/Standards/COMPOSITION_STANDARD.md. No changes required.
"""
