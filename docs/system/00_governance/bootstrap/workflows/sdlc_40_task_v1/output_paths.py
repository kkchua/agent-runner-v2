"""Output paths for sdlc_40_task_v1 workflow.

This module defines the artifact path mappings for the task workflow,
...
def build_output_paths(*, job_id: str = "{job_id}", mode: str = "{mode}") -> dict[str, str]:
    """Build output path mappings for sdlc_40_task_v1 workflow.
    ...
    return {
        # Task document (draft stage)
        "TASK_FILE": f"{delivery_base}/tasks/TASK-{date_str}-001-001_{{slug}}.md",

        # Review document (evidence artifact)
        "REVIEW_FILE_SUGGESTED": f"{delivery_base}/tasks/{job_id}-task-review.md",
    }
