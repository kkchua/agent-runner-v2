...
scan_policy: "include"
scan_reason: "Layer 1 document authority standard"
layer: "layer1"
...
This document defines the authority vocabulary, ownership matrix,
promotion constraints, and permanent-versus-temporary artifact rules for
the ecosystem.

Every governed document has an owner, a classification, and a scope.
Authority rules prevent two common failures:
...
The following authority values define who owns the truth of a document.
...
## Permanent and Temporary Artifacts

### Permanent Artifacts

Permanent artifacts are documents that form part of the governed
documentation set for a layer. They carry `doc_type` values such as
`system`, `platform_standard`, `bundle_definition`, or `workflow_output`
with `authority` values that identify the canonical owner.

### Temporary Artifacts

Temporary artifacts are evidence documents produced during review,
validation, or audit steps. They carry `doc_type` values such as
`review_artifact`, `validation_artifact`, or `audit_artifact` with
`authority` values of `workflow-generated` or `derived`.

Temporary artifacts must never be:

- included in the permanent governed document set
- presented as constitutional authority
- used as substitutes for permanent standards
...
Examples of promotion that is NOT automatic:
...
- a review artifact does not become a governance standard because it was
  accepted during workflow approval
...
- a document marked `human-authored` that was produced by a workflow

## Inheritance Rules

The authority vocabulary inherits downward across layers:

- Layer 1 defines the common authority values and baseline vocabulary
