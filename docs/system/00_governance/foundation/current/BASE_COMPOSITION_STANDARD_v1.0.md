...
| 2 | Workflow Package | `workflow.toml`, `context_extensions.py`, `actions.py`, `prompts/`, `README.md` | The executable workflow package. The `workflow.toml` IS the default implementation -- every step has its prompt or action assigned. Alternative implementations use `impls/{name}/impl.yaml` to override only the steps that differ (see Section 13.8). |
...
- `workflow.toml` -- the step sequence AND default implementation (every step has prompt= or action= assigned)
- `impls/` -- contains alternative runtime implementations (optional, only when variants are needed)
...
Implementations are selected at workflow invocation time:
