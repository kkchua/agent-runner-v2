...
|----------|-----------|---------|
| `load_model_mapping` | `(path)` | Load the model mapping file. |
| `resolve_coder` | `(name)` | Resolve a coder name into a full invocation config. |
| `get_api_key` | `(coder_config)` | Retrieve the API key for a coder config from environment variables. |
