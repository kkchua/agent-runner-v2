...
|-------|---------|-------------|
| `PreflightBlockedError` | Raised when a preflight check blocks step execution (e.g. artifact status not approved). | |
| `MetaJsonMissingError` | Raised when the coder did not write the expected meta.json sidecar after invocation. | |
| `MetaJsonInvalidError` | Raised when meta.json exists but fails schema validation. | |
| `ArtifactMissingError` | Raised when coder_result.artifacts references paths that don't exist on disk. | |
