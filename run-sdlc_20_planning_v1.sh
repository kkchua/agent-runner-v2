#!/usr/bin/env bash
# run-sdlc_20_planning_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_20_planning_v1 workflow:
...
TEMPLATE_GROUP="sdlc_20_planning_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
if [[ -n "${REQ_FILE}" ]]; then
    ...
    if [[ ! -f "${REQ_FILE_PATH}" ]]; then
        echo "ERROR: Req File file not found: ${REQ_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set REQ_FILE=${REQ_FILE_PATH}"
fi
...
[[ -n "${REQ_FILE}" ]] && echo " Req File:  ${REQ_FILE}"
