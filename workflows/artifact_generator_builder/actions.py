...
# ---------------------------------------------------------------------------
# Known pip-package → import-name mappings for common libraries.
# Used by check_capability_gaps when the Analysis JSON does not include
# an explicit "import_name" field.
# ---------------------------------------------------------------------------
...
def _extract_import_name(gap: dict) -> str | None:
    """Derive the Python import name from a capability_gap entry.

    Checks for an explicit ``import_name`` field first, then falls back
    to the pip package name extracted from ``proposed_solution``, and
    finally consults the ``_PIP_TO_IMPORT_NAME`` mapping.
    """
    explicit = gap.get("import_name", "")
    if explicit:
        return explicit.strip() or None

    solution = gap.get("proposed_solution", "")
    parts = solution.split()
    for i, part in enumerate(parts):
        if part in ("install", "pip install"):
            pkg = parts[i + 1] if i + 1 < len(parts) else ""
            pkg = pkg.strip().split("==")[0].split(">=")[0].split("<=")[0]
            if pkg:
                return _PIP_TO_IMPORT_NAME.get(pkg, pkg)
    return None


@action("check_capability_gaps")
def check_capability_gaps(*, context, state, step_cfg, project_root):
    """Check whether blocking capability gaps have been resolved.

    Reads the Analysis JSON's ``capability_gaps`` array. For each gap
    where ``blocking`` is true, attempts to import the required library.
    If any library is missing, returns REJECTED with install instructions
    so the job routes to AWAITING_INTERVENTION.

    When all blocking gaps are satisfied (libraries importable), returns
    APPROVED and the pipeline proceeds.
    """
    artifacts = state.get("artifacts", {})
    analysis_path = artifacts.get("ANALYSIS_JSON_FILE", "")
    ...
    if not analysis_path:
        return ActionResult(
            status="REJECTED",
            remark="ANALYSIS_JSON_FILE artifact not found — cannot check capability gaps.",
            artifacts={},
            reject_code="MISSING_ANALYSIS_JSON",
        )
    ...
    if not analysis_file.exists():
        return ActionResult(
            status="REJECTED",
            remark=f"Analysis JSON not found at {analysis_path}",
            artifacts={},
            reject_code="MISSING_ANALYSIS_JSON",
        )
    ...
    try:
        ...
        return ActionResult(
            status="REJECTED",
            remark=f"Cannot read Analysis JSON: {exc}",
            artifacts={},
            reject_code="INVALID_ANALYSIS_JSON",
        )
    ...
    if not blocking_gaps:
        return ActionResult(
            status="APPROVED",
            remark="No blocking capability gaps detected. Proceeding with pipeline.",
            artifacts={},
        )

    unmet: list[dict[str, str]] = []
    for gap in blocking_gaps:
        import_name = _extract_import_name(gap)
        if import_name is None:
            unmet.append({
                "requirement": gap.get("requirement", "unknown"),
                "solution": gap.get("proposed_solution", "unknown"),
                "reason": "Could not determine import name from capability_gap entry",
            })
            continue
        try:
            ...
            unmet.append({
                "requirement": gap.get("requirement", "unknown"),
                "solution": gap.get("proposed_solution", "unknown"),
                "reason": f"Library '{import_name}' is not installed",
            })

    if unmet:
        lines = ["Blocking capability gaps — required libraries not installed:"]
        for item in unmet:
            lines.append(f"  - {item['reason']}")
            lines.append(f"    Requirement: {item['requirement']}")
            lines.append(f"    Fix: {item['solution']}")
        lines.append("")
        lines.append("Install the required libraries and resume the job.")

        return ActionResult(
            status="REJECTED",
            remark="\n".join(lines),
            artifacts={},
            reject_code="CAPABILITY_GAP_BLOCKING",
        )

    resolved_names = [
        _extract_import_name(g) for g in blocking_gaps
    ]
    return ActionResult(
        status="APPROVED",
        remark=f"All {len(blocking_gaps)} blocking capability gap(s) resolved "
               f"(libraries importable: {', '.join(n for n in resolved_names if n)}).",
        artifacts={},
    )
