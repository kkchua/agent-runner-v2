...
class Sdlc80ReviewExtensions(WorkflowExtensions):
    """Workflow extension hooks for sdlc_80_review_v1."""
    ...
    def register_artifact_keys(self, *, job_id: str = "{job_id}", mode: str = "{mode}") -> dict[str, str]:
        ...
        return {
            "REV_FILE": f"{SDLC_DELIVERY_BASE}/80_reviews/REV-{date_str}-001_{{slug}}.md",
            "MEM_FILE": f"{SDLC_DELIVERY_BASE}/80_reviews/MEM-{date_str}-001_{{slug}}.md",
            "CLOSE_FILE": f"{SDLC_DELIVERY_BASE}/80_reviews/CLOSE-{date_str}-001_{{slug}}.md",
    ...
    def build_context_extensions(self, *, state: dict[str, Any], step: str, step_cfg: dict[str, Any], ctx: dict[str, str], project_root: Path | None = None) -> dict[str, str]:
        ...
        slug = _extract_slug_from_path(artifacts.get("VAL_FILE", ""))
