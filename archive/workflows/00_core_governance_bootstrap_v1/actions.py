...
def _extra_core_governance_checks(*, project_root: Path) -> list[dict[str, str | bool]]:
    ...
    runtime_text = docs["RUNTIME_GOVERNANCE.md"].read_text(encoding="utf-8")
    ...
    add(
        "docs/system/00_governance/bootstrap/SYSTEM_OVERVIEW.md" not in runtime_text
        and "docs/system/00_governance/bootstrap/SYSTEM_CONTEXT.md" not in runtime_text
        and "docs/system/00_governance/bootstrap/DEVELOPER_GUIDE.md" not in runtime_text,
        "runtime_governance_scope",
        docs["RUNTIME_GOVERNANCE.md"],
        "RUNTIME_GOVERNANCE must define steady-state runtime governance and must not present repo-analysis docs under `docs/system/00_governance/bootstrap/` as active Layer 1 artifacts.",
    )
    add(
        "plugin workflow bundle" in runtime_text.lower() or "plugin bundle" in runtime_text.lower(),
        "plugin_bundle_contract",
        docs["RUNTIME_GOVERNANCE.md"],
        "RUNTIME_GOVERNANCE must recognize plugin workflow bundles generically, including the possibility of multi-workflow bundles.",
    )

    known_names = sorted(name for name in registry_names if name)
    for bad_name in ("40_task_execution_v1", "41_bug_fix_intake_v1"):
        add(
            bad_name not in runtime_text and bad_name not in tax_text,
    ...
    add(
        "20_initiative_intake_v1" in registry_names or "20_initiative_intake_v1" not in runtime_text,
        "registry_workflow_name",
        docs["RUNTIME_GOVERNANCE.md"],
