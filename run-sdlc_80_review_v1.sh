#!/usr/bin/env bash
# run-sdlc_80_review_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_80_review_v1 workflow:
...
TEMPLATE_GROUP="sdlc_80_review_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
if [[ -n "${VAL_FILE}" ]]; then
    ...
    if [[ ! -f "${VAL_FILE_PATH}" ]]; then
        echo "ERROR: Val File file not found: ${VAL_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set VAL_FILE=${VAL_FILE_PATH}"
fi
...
[[ -n "${VAL_FILE}" ]] && echo " Val File:  ${VAL_FILE}"
