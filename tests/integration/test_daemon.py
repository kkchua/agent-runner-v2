...
def test_run_supervisor_persists_backend_linkage_and_accepts_awaiting_human(monkeypatch, tmp_path):
    sync_calls: list[dict] = []
    claims = [
        {
            'run': {'id': 'run-2', 'run_code': 'RUN-2'},
            'step_run': {'id': 'step-2', 'step_name': 'audit_docs'},
            'step_execution_spec': {'template_group': 'initiative_intake_v1'},
        },
        {'step_run': None},
    ]
    ...
    def fake_spawn_child(*, claim, runtime_root, cli_pythonpath, logger, backend_url, step_spec_source):
        child_dir = runtime_root / 'step-2'
        child_dir.mkdir(parents=True, exist_ok=True)
        result_path = child_dir / 'result.json'
        result_path.write_text(json.dumps({
            'status': 'completed',
            'outcome': 'approved',
            'step_name': 'audit_docs',
            'artifacts': {},
            'diagnostics': {},
            'failure': None,
        }), encoding='utf-8')
        log_path = child_dir / 'child.log'
        log_path.write_text('started\n', encoding='utf-8')
        job_path = fake_job_dir('initiative_intake_v1', 'RUN-2')
        job_path.mkdir(parents=True, exist_ok=True)
        (job_path / 'job.json').write_text(json.dumps({
            'job_status': 'WAITING_FOR_HUMAN_APPROVAL',
            'current_step': 'audit_docs',
            'completed_steps': ['pre_init', 'review_docs', 'audit_docs'],
            'project_root': str(tmp_path),
            'artifacts': {},
        }), encoding='utf-8')
        return daemon_module.ChildExecution(
            ...
            step_run_id='step-2',
            step_name='audit_docs',
            run_payload={'id': 'run-2', 'run_code': 'RUN-2'},
            step_run_payload={'id': 'step-2', 'step_name': 'audit_docs'},
            request_payload={'template_group': 'initiative_intake_v1', 'job_id': 'RUN-2'},
            request_path=child_dir / 'request.json',
            result_path=result_path,
            combined_log_path=log_path,
            child_event_log_path=child_dir / 'child-events.jsonl',
            process=DummyProc([None, 0]),
            started_at_monotonic=0.0,
            started_at_iso='2026-07-18T00:00:00+00:00',
            state='running',
        )
