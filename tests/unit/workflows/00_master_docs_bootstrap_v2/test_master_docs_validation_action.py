...
def _load_workflow_actions_module():
    ...
    spec = importlib.util.spec_from_file_location("tests.workflow_00_master_docs_bootstrap_v2_actions", module_path)
...
def test_should_validate_frontmatter_skips_non_document_artifacts() -> None:
    assert workflow_actions._should_validate_frontmatter(
        ...
        rel_path="docs/system/00_governance/bootstrap/JOB-bootstrap-validation.md",
    ) is False
    assert workflow_actions._should_validate_frontmatter(
        artifact_key="CODEBASE_SCAN_SNAPSHOT",
        rel_path="docs/codebase/04_changes/JOB-bootstrap-snapshot.json",
    ) is False
    assert workflow_actions._should_validate_frontmatter(
        artifact_key="SYSTEM_DOCS_INDEX",
        rel_path="docs/system/00_governance/bootstrap/README.md",
    ) is True
...
def test_validate_system_docs_ignores_existing_validation_artifact_frontmatter(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "workflow_name": kwargs.get("workflow_name", "00_master_docs_bootstrap_v2"),
    ...
    required_sections = workflow_actions.SYSTEM_DOC_REQUIRED_SECTIONS
    for artifact_key, rel_path in output_paths.items():
        ...
        if rel_path.endswith(".json"):
            path.write_text("{}\n", encoding="utf-8")
            continue
        if artifact_key == "BOOTSTRAP_SUMMARY":
            continue
        if artifact_key == "SYSTEM_DOCS_VALIDATION":
            path.write_text("# Existing validation\n\nNo frontmatter here.\n", encoding="utf-8")
            continue
...
def test_validate_system_docs_reports_exact_frontmatter_failure(tmp_path, monkeypatch) -> None:
    ...
    assert output_paths["PROJECT_ANALYSIS"] in rendered
