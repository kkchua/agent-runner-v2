#!/usr/bin/env bash
# submit-sdlc_50_implementation_v1.sh - Edit the variables below, then run.
#
# Submits a new backend job for the sdlc_50_implementation_v1 workflow:
...
if [[ -n "${TASK_FILE}" ]]; then
    ...
    if [[ ! -f "${TASK_FILE_PATH}" ]]; then
        echo "ERROR: TASK_FILE file not found: ${TASK_FILE_PATH}"
        exit 1
    fi
    INPUT_FLAGS="--input TASK_FILE=${TASK_FILE_PATH}"
fi
