#!/usr/bin/env bash
# submit-sdlc_80_review_v1.sh - Edit the variables below, then run.
#
# Submits a new backend job for the sdlc_80_review_v1 workflow:
#   Generates final review, memory, and closure documents.
...
WORKFLOW_NAME="sdlc_80_review_v1"
INITIATIVE_ID=""
WORKER_LABEL="live"
WORKER_ID=""
BACKEND_URL=""
...
# VAL_FILE input file (filename only)
# Must exist in docs/repo/agent_runner/sdlc/delivery/70_validations/
VAL_FILE="VAL-20260722-001_console-sdlc10-support.md"
...
INPUT_FLAGS=""
if [[ -n "${VAL_FILE}" ]]; then
    VAL_FILE_PATH="$SCRIPT_DIR/docs/repo/agent_runner/sdlc/delivery/70_validations/${VAL_FILE}"
    if [[ ! -f "${VAL_FILE_PATH}" ]]; then
        echo "ERROR: VAL_FILE file not found: ${VAL_FILE_PATH}"
        exit 1
    fi
    INPUT_FLAGS="--input VAL_FILE=${VAL_FILE_PATH}"
fi
...
[[ -n "${VAL_FILE}" ]] && echo " VAL_FILE:  ${VAL_FILE}"
echo "==========================================================================="
echo ""
