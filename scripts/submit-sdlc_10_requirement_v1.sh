#!/usr/bin/env bash
# submit-sdlc_10_requirement_v1.sh - Edit the variables below, then run.
#
# Submits a new backend job for the sdlc_10_requirement_v1 workflow:
#   Generates structured requirements from approved initiative.
...
INIT_FILE="INIT-20260723-001_console-sdlc10-support.md"
...
if [[ -n "${INIT_FILE}" ]]; then
    ...
    if [[ ! -f "${INIT_FILE_PATH}" ]]; then
        echo "ERROR: INIT_FILE file not found: ${INIT_FILE_PATH}"
        exit 1
    fi
    INPUT_FLAGS="--input INIT_FILE=${INIT_FILE_PATH}"
fi
...
[[ -n "${INIT_FILE}" ]] && echo " INIT_FILE:  ${INIT_FILE}"
echo "==========================================================================="
echo ""
