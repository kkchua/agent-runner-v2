@echo off
REM submit-sdlc_70_validation_v1.bat - Edit the variables below, then run.
REM
REM Submits a new backend job for the sdlc_70_validation_v1 workflow:
REM   ukbe-run-agent submit --workflow-name sdlc_70_validation_v1 ...
REM
REM This workflow generates a validation report from approved execution docs,
REM producing an approved VAL_FILE document.
...
set "WORKFLOW_NAME=sdlc_70_validation_v1"
set "INITIATIVE_ID="
...
set "INPUT_FLAGS="
if not "!EXEC_FILE!"=="" (
    set "EXEC_PATH=%~dp0docs\repo\agent_runner\sdlc\delivery\60_executions\!EXEC_FILE!"
    if not exist "!EXEC_PATH!" (
        echo ERROR: Execution file not found: !EXEC_PATH!
        pause
        exit /b 1
    )
    set "INPUT_FLAGS=--input EXEC_FILE=!EXEC_PATH!"
)
