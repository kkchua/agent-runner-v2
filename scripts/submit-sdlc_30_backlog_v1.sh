#!/usr/bin/env bash
# submit-sdlc_30_backlog_v1.sh - Edit the variables below, then run.
#
# Submits a new backend job for the sdlc_30_backlog_v1 workflow:
...
if [[ -n "${PLAN_FILE}" ]]; then
    ...
    if [[ ! -f "${PLAN_FILE_PATH}" ]]; then
        echo "ERROR: PLAN_FILE file not found: ${PLAN_FILE_PATH}"
        exit 1
    fi
    INPUT_FLAGS="--input PLAN_FILE=${PLAN_FILE_PATH}"
fi
...
[[ -n "${PLAN_FILE}" ]] && echo " PLAN_FILE:  ${PLAN_FILE}"
echo "==========================================================================="
echo ""
