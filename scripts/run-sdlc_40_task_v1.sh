#!/usr/bin/env bash
# run-sdlc_40_task_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_40_task_v1 workflow:
...
TEMPLATE_GROUP="sdlc_40_task_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
if [[ -n "${BACKLOG_FILE}" ]]; then
    ...
    if [[ ! -f "${BACKLOG_FILE_PATH}" ]]; then
        echo "ERROR: Backlog File file not found: ${BACKLOG_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set BACKLOG_FILE=${BACKLOG_FILE_PATH}"
fi
...
[[ -n "${BACKLOG_FILE}" ]] && echo " Backlog File:  ${BACKLOG_FILE}"
