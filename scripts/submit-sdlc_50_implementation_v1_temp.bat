@echo off
REM submit-sdlc_50_implementation_v1.bat - Edit the variables below, then run.
REM
REM Submits a new backend job for the sdlc_50_implementation_v1 workflow:
REM   ukbe-run-agent submit --workflow-name sdlc_50_implementation_v1 ...
REM
REM This workflow generates an implementation plan from approved task,
REM producing an approved IMPL_FILE document.
...
set "WORKFLOW_NAME=sdlc_50_implementation_v1"
set "INITIATIVE_ID="
...
set "TASK_FILE=TASK-20260814-001-01_gen-media-content-scaffolding.md"
...
set "INPUT_FLAGS="
if not "!TASK_FILE!"=="" (
    set "TASK_PATH=%~dp0docs\repo\agent_runner\sdlc\delivery\40_tasks\!TASK_FILE!"
    if not exist "!TASK_PATH!" (
        echo ERROR: Task file not found: !TASK_PATH!
        pause
        exit /b 1
    )
    set "INPUT_FLAGS=--input TASK_FILE=!TASK_PATH!"
)
