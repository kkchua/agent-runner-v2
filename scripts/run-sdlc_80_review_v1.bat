@echo off
REM run-sdlc_80_review_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_80_review_v1 workflow (plugin package):
REM   Generates review, memory, and closure documents from approved validation.
REM   Loads the workflow definition from workflows/sdlc_80_review_v1/workflow.toml.
REM
REM Runtime jobs and sidecars are stored under %USERPROFILE%\.ukbe-runner\jobs\
REM
REM Typical workflow:
REM   1. Edit the variables below (especially VAL_FILE)
...
set "TEMPLATE_GROUP=sdlc_80_review_v1"
...
set "SEED_FLAGS="
if not "!VAL_FILE!"=="" (
    set "VAL_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\70_validations\!VAL_FILE!"
    if not exist "!VAL_PATH!" (
        echo ERROR: Validation file not found: !VAL_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set VAL_FILE=!VAL_PATH!"
)
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-80-review-status-!RANDOM!.txt"
