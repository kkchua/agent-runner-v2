@echo off
REM submit-sdlc_20_planning_v1.bat - Edit the variables below, then run.
REM
REM Submits a new backend job for the sdlc_20_planning_v1 workflow:
REM   ukbe-run-agent submit --workflow-name sdlc_20_planning_v1 ...
REM
REM This workflow generates a solution architecture plan from approved requirements,
REM producing an approved PLAN_FILE document.
...
set "WORKFLOW_NAME=sdlc_20_planning_v1"
set "INITIATIVE_ID="
...
set "INPUT_FLAGS="
if not "!REQ_FILE!"=="" (
    set "REQ_PATH=%~dp0docs\repo\agent_runner\sdlc\delivery\10_requirements\!REQ_FILE!"
    if not exist "!REQ_PATH!" (
        echo ERROR: Requirements file not found: !REQ_PATH!
        pause
        exit /b 1
    )
    set "INPUT_FLAGS=--input REQ_FILE=!REQ_PATH!"
)
