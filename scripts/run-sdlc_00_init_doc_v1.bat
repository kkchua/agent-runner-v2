@echo off
REM run-sdlc_00_init_doc_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_00_init_doc_v1 workflow (plugin package):
REM   Captures and structures initiative from draft input.
REM   Loads the workflow definition from workflows/sdlc_00_init_doc_v1/workflow.toml.
REM
...
set "TEMPLATE_GROUP=sdlc_00_init_doc_v1"
...
REM Must exist in docs/repo/agent_runner/sdlc/delivery/00_draft_initiatives/
...
if not "!DRAFT_INIT_FILE!"=="" (
    set "DRAFT_INIT_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\00_draft_initiatives\!DRAFT_INIT_FILE!"
    if not exist "!DRAFT_INIT_PATH!" (
        echo ERROR: Draft initiative file not found: !DRAFT_INIT_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set DRAFT_INIT_FILE=!DRAFT_INIT_PATH!"
)
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-00-init-status-!RANDOM!.txt"
