@echo off
REM run-sdlc_00_delivery_scaffold_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_00_delivery_scaffold_v1 workflow (plugin package):
REM   Generates the Layer 2 platform core constitution for agent-runner-v2.
REM   Loads the workflow definition from workflows/sdlc_00_delivery_scaffold_v1/workflow.toml.
REM
...
set "TEMPLATE_GROUP=sdlc_00_delivery_scaffold_v1"

REM Job ID to resume (leave blank to auto-create a new job)
set "JOB_ID=SDLC00SCF-GEN-20260722-006"
