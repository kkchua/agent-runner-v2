@echo off
REM submit-sdlc_00_init_doc_v1.bat - Edit the variables below, then run.
REM
REM Submits a new backend job for the sdlc_00_init_doc_v1 workflow:
REM   ukbe-run-agent submit --workflow-name sdlc_00_init_doc_v1 ...
REM
REM This workflow captures and structures initiative from draft input,
REM producing an approved INIT_FILE document.
...
set "WORKFLOW_NAME=sdlc_00_init_doc_v1"
set "INITIATIVE_ID="
...
if not "!DRAFT_INIT_FILE!"=="" (
    set "DRAFT_INIT_PATH=%~dp0docs\repo\agent_runner\sdlc\delivery\00_draft_initiatives\!DRAFT_INIT_FILE!"
    if not exist "!DRAFT_INIT_PATH!" (
        echo ERROR: Draft initiative file not found: !DRAFT_INIT_PATH!
        pause
        exit /b 1
    )
    set "INPUT_FLAGS=--input DRAFT_INIT_FILE=!DRAFT_INIT_PATH!"
)
