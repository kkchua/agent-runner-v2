#!/usr/bin/env bash
# run-sdlc_00_init_doc_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_00_init_doc_v1 workflow:
...
TEMPLATE_GROUP="sdlc_00_init_doc_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
if [[ -n "${DRAFT_INIT_FILE}" ]]; then
    ...
    if [[ ! -f "${DRAFT_INIT_FILE_PATH}" ]]; then
        echo "ERROR: Draft Init file not found: ${DRAFT_INIT_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set DRAFT_INIT_FILE=${DRAFT_INIT_FILE_PATH}"
fi
...
[[ -n "${DRAFT_INIT_FILE}" ]] && echo " Draft Init:  ${DRAFT_INIT_FILE}"
