@echo off
REM run-sdlc_00_codebase_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_00_codebase_v1 workflow (plugin package):
REM   Periodic sync of repository code to codebase documentation.
REM   Loads the workflow definition from workflows/sdlc_00_codebase_v1/workflow.toml.
REM
...
set "TEMPLATE_GROUP=sdlc_00_codebase_v1"

REM Job ID to resume (leave blank to auto-create a new job)
set "JOB_ID=SDLC00CB-GEN-20260723-002"
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc00cb-status-!RANDOM!.txt"
