@echo off
REM run-sdlc_50_implementation_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_50_implementation_v1 workflow (plugin package):
REM   Generates implementation plan from approved task document.
REM   Loads the workflow definition from workflows/sdlc_50_implementation_v1/workflow.toml.
REM
REM Runtime jobs and sidecars are stored under %USERPROFILE%\.ukbe-runner\jobs\
REM
REM Typical workflow:
REM   1. Edit the variables below (especially TASK_FILE)
...
set "TEMPLATE_GROUP=sdlc_50_implementation_v1"
...
set "SEED_FLAGS="
if not "!TASK_FILE!"=="" (
    set "TASK_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\40_tasks\!TASK_FILE!"
    if not exist "!TASK_PATH!" (
        echo ERROR: Task file not found: !TASK_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set TASK_FILE=!TASK_PATH!"
)
...
if not "!TASK_FILE!"=="" echo  Task File:       !TASK_FILE!
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-50-implementation-status-!RANDOM!.txt"
