#!/usr/bin/env bash
# run-sdlc_70_validation_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_70_validation_v1 workflow:
#   Generates validation report for initiative completion.
...
TEMPLATE_GROUP="sdlc_70_validation_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
# EXEC_FILE input file (filename only)
# Must exist in docs/repo/agent_runner/sdlc/delivery/60_executions/
EXEC_FILE="EXEC-20260722-001_console-sdlc10-support.md"
...
SEED_FLAGS=""
if [[ -n "${EXEC_FILE}" ]]; then
    EXEC_FILE_PATH="$SCRIPT_DIR/docs/repo/agent_runner/sdlc/delivery/60_executions/${EXEC_FILE}"
    if [[ ! -f "${EXEC_FILE_PATH}" ]]; then
        echo "ERROR: Exec File file not found: ${EXEC_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set EXEC_FILE=${EXEC_FILE_PATH}"
fi
...
[[ -n "${EXEC_FILE}" ]] && echo " Exec File:  ${EXEC_FILE}"
