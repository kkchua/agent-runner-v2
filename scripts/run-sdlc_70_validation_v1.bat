@echo off
REM run-sdlc_70_validation_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_70_validation_v1 workflow (plugin package):
REM   Generates validation report from approved execution docs.
REM   Loads the workflow definition from workflows/sdlc_70_validation_v1/workflow.toml.
REM
REM Runtime jobs and sidecars are stored under %USERPROFILE%\.ukbe-runner\jobs\
REM
REM Typical workflow:
REM   1. Edit the variables below (especially EXEC_FILE)
...
set "TEMPLATE_GROUP=sdlc_70_validation_v1"
...
REM Approved execution document (filename only, e.g., EXEC-20260722-001_console-sdlc10-support.md)
REM Must exist in docs/repo/agent_runner/sdlc/delivery/60_executions/
set "EXEC_FILE=EXEC-20260722-001_console-sdlc10-support.md"
...
set "SEED_FLAGS="
if not "!EXEC_FILE!"=="" (
    set "EXEC_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\60_executions\!EXEC_FILE!"
    if not exist "!EXEC_PATH!" (
        echo ERROR: Execution file not found: !EXEC_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set EXEC_FILE=!EXEC_PATH!"
)
...
if not "!EXEC_FILE!"=="" echo  Exec File:       !EXEC_FILE!
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-70-validation-status-!RANDOM!.txt"
