#!/usr/bin/env bash
# submit-sdlc_60_execution_v1.sh - Edit the variables below, then run.
#
# Submits a new backend job for the sdlc_60_execution_v1 workflow:
...
if [[ -n "${IMPL_FILE}" ]]; then
    ...
    if [[ ! -f "${IMPL_FILE_PATH}" ]]; then
        echo "ERROR: IMPL_FILE file not found: ${IMPL_FILE_PATH}"
        exit 1
    fi
    INPUT_FLAGS="--input IMPL_FILE=${IMPL_FILE_PATH}"
fi
...
[[ -n "${IMPL_FILE}" ]] && echo " IMPL_FILE:  ${IMPL_FILE}"
echo "==========================================================================="
echo ""
