#!/usr/bin/env bash
# run-sdlc_60_execution_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_60_execution_v1 workflow:
#   Generates execution record for task completion.
...
TEMPLATE_GROUP="sdlc_60_execution_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
# IMPL_FILE input file (filename only)
# Must exist in docs/repo/agent_runner/sdlc/delivery/50_implementations/
IMPL_FILE="IMPL-20260722-001_console-sdlc10-support.md"
...
SEED_FLAGS=""
if [[ -n "${IMPL_FILE}" ]]; then
    IMPL_FILE_PATH="$SCRIPT_DIR/docs/repo/agent_runner/sdlc/delivery/50_implementations/${IMPL_FILE}"
    if [[ ! -f "${IMPL_FILE_PATH}" ]]; then
        echo "ERROR: Impl File file not found: ${IMPL_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set IMPL_FILE=${IMPL_FILE_PATH}"
fi
...
[[ -n "${IMPL_FILE}" ]] && echo " Impl File:  ${IMPL_FILE}"
