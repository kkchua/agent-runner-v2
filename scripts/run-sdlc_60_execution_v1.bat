@echo off
REM run-sdlc_60_execution_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the sdlc_60_execution_v1 workflow (plugin package):
REM   Executes tasks following approved implementation docs.
REM   Loads the workflow definition from workflows/sdlc_60_execution_v1/workflow.toml.
REM
REM Runtime jobs and sidecars are stored under %USERPROFILE%\.ukbe-runner\jobs\
REM
REM Typical workflow:
REM   1. Edit the variables below (especially IMPL_FILE)
...
set "TEMPLATE_GROUP=sdlc_60_execution_v1"
...
set "SEED_FLAGS="
if not "!IMPL_FILE!"=="" (
    set "IMPL_PATH=%CD%\docs\repo\agent_runner\sdlc\delivery\50_implementations\!IMPL_FILE!"
    if not exist "!IMPL_PATH!" (
        echo ERROR: Implementation file not found: !IMPL_PATH!
        pause
        exit /b 1
    )
    set "SEED_FLAGS=--set IMPL_FILE=!IMPL_PATH!"
)
...
if not "!IMPL_FILE!"=="" echo  Impl File:       !IMPL_FILE!
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-sdlc-60-execution-status-!RANDOM!.txt"
