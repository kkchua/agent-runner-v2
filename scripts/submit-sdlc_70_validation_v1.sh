#!/usr/bin/env bash
# submit-sdlc_70_validation_v1.sh - Edit the variables below, then run.
#
# Submits a new backend job for the sdlc_70_validation_v1 workflow:
...
if [[ -n "${EXEC_FILE}" ]]; then
    ...
    if [[ ! -f "${EXEC_FILE_PATH}" ]]; then
        echo "ERROR: EXEC_FILE file not found: ${EXEC_FILE_PATH}"
        exit 1
    fi
    INPUT_FLAGS="--input EXEC_FILE=${EXEC_FILE_PATH}"
fi
...
[[ -n "${EXEC_FILE}" ]] && echo " EXEC_FILE:  ${EXEC_FILE}"
echo "==========================================================================="
echo ""
