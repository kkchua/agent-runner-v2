#!/usr/bin/env bash
# run-sdlc_50_implementation_v1.sh - Edit the variables below, then run.
#
# Runs ukbe-run-agent for the sdlc_50_implementation_v1 workflow:
#   Generates implementation plan for task execution.
...
TEMPLATE_GROUP="sdlc_50_implementation_v1"
JOB_ID=""
DRY_RUN="false"
NEW_JOB="false"
MODE="manual"
...
# TASK_FILE input file (filename only)
# Must exist in docs/repo/agent_runner/sdlc/delivery/40_tasks/
TASK_FILE="TASK-20260722-001_console-sdlc10-support.md"
...
SEED_FLAGS=""
if [[ -n "${TASK_FILE}" ]]; then
    TASK_FILE_PATH="$SCRIPT_DIR/docs/repo/agent_runner/sdlc/delivery/40_tasks/${TASK_FILE}"
    if [[ ! -f "${TASK_FILE_PATH}" ]]; then
        echo "ERROR: Task File file not found: ${TASK_FILE_PATH}"
        exit 1
    fi
    SEED_FLAGS="--set TASK_FILE=${TASK_FILE_PATH}"
fi
