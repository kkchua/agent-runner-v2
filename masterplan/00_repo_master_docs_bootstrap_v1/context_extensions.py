...
def build_context_extensions(
    *,
    ...
    For every artifact in MASTER_DOCS_OUTPUT_PATHS, injects:
      - ``{KEY}``         → resolved output path (absolute if project_root provided)
    ...
    """
    job_id = str(state.get("job_id") or "00RMD").strip()
    ...
    for artifact_key, output_path in output_paths.items():
        ...
        if project_root and not Path(output_path).is_absolute():
            resolved_path = str((project_root / output_path).resolve()).replace("\\", "/")
        else:
