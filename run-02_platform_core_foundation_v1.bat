@echo off
REM run-02_platform_core_foundation_v1.bat - Edit the variables below, then double-click (or run).
REM
REM Runs ukbe-run-agent for the 02_platform_core_foundation_v1 workflow (plugin package):
REM   Generates the Layer 2 platform core constitution for agent-runner-v2.
REM   Loads the workflow definition from workflows/02_platform_core_foundation_v1/workflow.toml.
REM
...
if not "!JOB_ID!"=="" (
    ...
    set "STATUS_FILE=%CD%\temp\run-02-platform-core-status-!RANDOM!.txt"
