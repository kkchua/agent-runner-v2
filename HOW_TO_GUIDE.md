...
The `10_execution_scaffold_v1` workflow generates a complete delivery documentation system — templates, SOP, status rules, agent contracts, and codebase-documentation governance — into any target repository. This is the first step for any new project that wants to use the agent-runner-v2 orchestration system.
...
```bash
ukbe-run-agent run \
  --template-group 10_execution_scaffold_v1
```
...
The `delivery_planning_v1` workflow will validate its output (PLAN, TASK_GRAPH, TASK) against the templates generated by `10_execution_scaffold_v1` via the `template_ref` configuration.
...
This happens when running `delivery_planning_v1` or other workflows on a project that hasn't been scaffolded yet. Run `10_execution_scaffold_v1` first, or ensure the referenced files exist.
...
The `10_execution_scaffold_v1` workflow integrates with existing workflows through `template_ref`:
