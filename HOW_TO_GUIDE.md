...
### Agent Master Prompts Are Deprecated

The scaffold workflow does NOT generate `07_master_prompts/`. The workflow step definitions in `template_groups.py` serve as the agent contracts — each step config specifies the prompt template, coder, inputs/outputs, and behavior rules. Individual agent contracts (`08_agents/AGENT-*.md`) define role boundaries and handoff rules, not prompt content.
